...
contract IDOPool is Ownable, ReentrancyGuard {
     ...
     * @param _multiplier muliplier of participation Amount
     */
    ...
    constructor(
        ...
        require(_multiplier > 0, "_multiplier should be > zero");
    ...
    function claimableAmount(address _user) public view returns (uint256 _tokenAmount){
        ...
        if (block.timestamp < releaseTime || sale.tokenWithdrawnStatus) {
            return 0;
        }
    ...
    function buy(uint256 amount) external publicSaleActive nonReentrant {
        ...
        require(amount % multiplier == 0, 
                "Amount should be based on multiplier");
        ...
        uint256 allocatedToken = (amountAfterFee).mul(10 ** token.decimals()).div(price);
