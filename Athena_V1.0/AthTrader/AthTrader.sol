...
contract AthTrader {
     ...
     * @param fundingPeriodInDays_ funding period defined in days
     * @param tradingPeriodInDays_ trading period defined in days
    ...
    function initializeRound(
        address trader_,
        uint32 startTime_,
        uint32 fundingPeriodInDays_,
        uint32 tradingPeriodInDays_,
        ...
        fundingPeriod = fundingPeriodInDays_ * 86400;
        tradingPeriod = tradingPeriodInDays_ * 86400;
    ...
    function isFundingActiveForAthLevel(uint8 level_) public view returns(bool) {
        uint8 _elapsedDays = uint8((block.timestamp - fundingStartTime) / 86400); 

        if(level_ == 3) {
            return true;
        } else if(level_ == 2 && _elapsedDays > 0) {
            return true;
        } else if(level_ == 1 && _elapsedDays > 1) {
            return true;
        } else if(_elapsedDays > 2) {
            return true;    
        } else {
            return false;
        }
