...
contract AthTrader {
	...
	require(claimingPeriodInSeconds_ >= 14 days, "Invalid claiming period");
	 ...
	 */
    function getTradedTokenList() public view returns(address[] memory) {
        return tradedToken;
    }
     ...
     */
     function isTradedTokenConverted() public view returns(bool) {
         for (uint256 i = 0; i < tradedToken.length; i++) {
            if (IBEP20Token(tradedToken[i]).balanceOf(address(this)) > 0) {
                return false;
            }
         }
         return true;
    ...
    function harvestReward() external {
        ...
        // fee is not applicable to trader
        ...
        if(_fee > 0) {
            ...
            IBEP20Token(participationToken).transfer(owner, _fee.sub(_traderFee));
